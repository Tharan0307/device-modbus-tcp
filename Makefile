...
MICROSERVICES=cmd/device-modbus-tcp/device-modbus-tcp
.PHONY: $(MICROSERVICES)
...
GOFLAGS=-ldflags "-X github.com/edgexfoundry/device-modbus-tcp.Version=$(VERSION) \
...
cmd/device-modbus-tcp/device-modbus-tcp:
	CGO_ENABLED=0  go build $(GOFLAGS) -o $@ ./cmd/device-modbus-tcp

docker:
	docker build \
		-f cmd/device-modbus-tcp/Dockerfile \
		--label "git_sha=$(GIT_SHA)" \
		-t edgexfoundry/device-modbus-tcp:$(GIT_SHA) \
		-t edgexfoundry/device-modbus-tcp:$(DOCKER_TAG) \
		.
